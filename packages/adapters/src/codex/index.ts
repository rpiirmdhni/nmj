// Codex adapter — spawns OpenAI Codex CLI
...
export class CodexAdapter extends BaseAdapter {
  ...
  async execute(agent: Agent, message: string, context?: string, systemPrompt?: string): Promise<string> {
    ...
    if (config.dangerouslyBypassSandbox !== false) {
      args.push("--dangerously-bypass-approvals-and-sandbox");
    }

    // Inject system prompt via temp file
    let tempFilePath: string | null = null;
    if (systemPrompt) {
      tempFilePath = this.writeTempFile(systemPrompt);
      args.push("-c", `experimental_instructions_file=${tempFilePath}`);
    }
  ...
  async testEnvironment(agent: Agent): Promise<AdapterEnvironmentTestResult> {
    ...
    checks.push({
      ...
      message: cliExists
        ? `Codex CLI found: ${command}`
        : `Codex CLI not found: ${command}`,
      hint: cliExists ? null : "Install with: npm install -g @openai/codex",
    });
    if (!cliExists) {
    ...
    checks.push({
      code: "auth",
      level: hasApiKey ? "info" : "error",
      message: hasApiKey
        ? "OPENAI_API_KEY is set"
        : "OPENAI_API_KEY is not set",
      hint: hasApiKey ? null : "Set OPENAI_API_KEY environment variable",
    });
    if (config.model) {
      checks.push({
        code: "model",
