// Process adapter — executes arbitrary shell commands
// System prompt: injected via message prefix
...
export class ProcessAdapter extends BaseAdapter {
  ...
  async execute(agent: Agent, message: string, context?: string, systemPrompt?: string): Promise<string> {
    ...
    if (result.timedOut) throw new Error(`Process timed out after ${timeoutSec}s`);
    if (result.exitCode !== 0) throw new Error(`Process exited with code ${result.exitCode}: ${result.stderr}`);
  ...
  async executeStreaming(agent: Agent, message: string, context: string | undefined, onChunk: (chunk: string) => void, systemPrompt?: string): Promise<string> {
    ...
    if (result.timedOut) throw new Error(`Process timed out after ${timeoutSec}s`);
    if (result.exitCode !== 0 && !fullText) throw new Error(`Process exited with code ${result.exitCode}: ${result.stderr}`);
    return fullText;
  }
  ...
  async testEnvironment(agent: Agent): Promise<AdapterEnvironmentTestResult> {
    ...
    const command = config.command;
    if (!command) {
      checks.push({ code: "command_missing", level: "error", message: "No command configured", hint: "Set 'command' in adapter config (e.g., 'bash', 'python', 'node')" });
      return this.createTestResult(this.type, "fail", checks);
    }
    const cmdExists = await this.checkCommandExists(command);
    checks.push({ code: "command_exists", level: cmdExists ? "info" : "error", message: cmdExists ? `Command found: ${command}` : `Command not found: ${command}`, hint: cmdExists ? null : `Ensure '${command}' is installed and in PATH` });
    if (config.cwd) { checks.push({ code: "cwd", level: "info", message: `Working directory: ${config.cwd}` }); }
