// OpenCode adapter — spawns OpenCode CLI
// System prompt: injected via message prefix (no native CLI flag available)
...
export class OpenCodeAdapter extends BaseAdapter {
  ...
  async execute(agent: Agent, message: string, context?: string, systemPrompt?: string): Promise<string> {
    ...
    if (result.timedOut) {
      throw new Error(`OpenCode timed out after ${timeoutSec}s`);
    }
    if (result.exitCode !== 0) {
      throw new Error(`OpenCode exited with code ${result.exitCode}: ${result.stderr}`);
    }
  ...
  async testEnvironment(agent: Agent): Promise<AdapterEnvironmentTestResult> {
    ...
    checks.push({
      ...
      message: cliExists
        ? `OpenCode CLI found: ${command}`
        : `OpenCode CLI not found: ${command}`,
      hint: cliExists ? null : "Install opencode-ai package",
    });
    if (!cliExists) {
    ...
    if (config.model) {
      ...
      checks.push({
        code: "model",
        level: "warn",
        message: "No model configured — will use OpenCode default",
      });
    }
